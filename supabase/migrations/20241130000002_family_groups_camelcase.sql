-- This migration works with existing camelCase schema
...
-- Add foreign key constraint to existing group_id column (keeping camelCase)
do $$
begin
  -- Add foreign key constraint if it doesn't exist
  if not exists (
    select 1 from information_schema.table_constraints 
    where constraint_name = 'boards_group_id_fkey'
    and table_name = 'boards'
  ) then
    alter table public.boards 
    add constraint boards_group_id_fkey 
...
create index if not exists boards_group_id_idx on public.boards(group_id);
...
-- Boards policies (using group_id instead of family_group_id)
