-- Fix RLS policies properly to avoid infinite recursion
-- The key issue: family_members INSERT policy was checking family_members table recursively
...
CREATE POLICY "family_members_select"
  ...
  USING (
    ...
    user_id = auth.uid()
    OR
    -- Can see members of groups you're in
    family_group_id IN (
...
-- INSERT: This is the critical one - avoid recursion!
-- Solution: Check against family_groups.created_by OR use a CTE
CREATE POLICY "family_members_insert"
  ON public.family_members FOR INSERT
  TO authenticated
  WITH CHECK (
    -- Allow if you're the creator of the group (no recursion!)
    family_group_id IN (
      SELECT id 
    ...
    OR
    -- Allow if you're already an owner/admin (uses SELECT which is allowed)
    family_group_id IN (
      SELECT fm.family_group_id 
      FROM public.family_members fm 
      WHERE fm.user_id = auth.uid() 
...
-- DELETE: Owners and admins can remove members, or users can remove themselves
...
CREATE POLICY "boards_insert"
  ...
  WITH CHECK (
    auth.uid() = user_id
    AND (
      -- If group_id is set, user must be a member
      group_id IS NULL
      OR group_id IN (
        SELECT family_group_id 
        FROM public.family_members 
...
-- UPDATE: Users can update their own boards and family group boards they're members of
...
-- DELETE: Users can delete their own boards and family group boards they're members of
